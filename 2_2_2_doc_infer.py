...
model_dbow.build_vocab([x for x in x_train])
...
for index, doc in enumerate(x_test):
    ...
    for ww in doc.words:
        try:
            if model_dbow.wv.get_vecattr(ww,"count"):
                words_.append(ww)
        except KeyError:
            if index == 0:
                print(ww," 0",end=" ")
...
ivec_list=[]
for index in range(len(docs_)):  
    ivec = model_dbow.infer_vector(doc_words=docs_[index], alpha=0.025)
    ivec_list.append(ivec)

test_vectors_dbow=np.array(ivec_list)
